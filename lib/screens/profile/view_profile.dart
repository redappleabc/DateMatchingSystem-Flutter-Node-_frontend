...
class _ViewProfileScreenState extends State<ViewProfileScreen> {
  ...
  Future getUserById() async {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      final args =
          ModalRoute.of(context)!.settings.arguments as UserTransforIdModel;
  ...
  Future saveRecord() async {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      final args =
          ModalRoute.of(context)!.settings.arguments as UserTransforIdModel;
      await Provider.of<RecordState>(context, listen: false)
          .saveRecord(args.id);
    });
  }
  ...
  Future sendPostMessage(int id) async {
    try {
      await Provider.of<PostState>(context, listen: false)
          .sendPostMessage(id, messageController.text);
  ...
  Future<void> sendMessage(
      int id, String name, String avatar, int age, int prefectureId) async {
                            ...
                            Container(
                                ...
                                decoration: BoxDecoration(
                                    ...
                                    image: DecorationImage(
                                        image: NetworkImage(
                                            "${dotenv.get('BASE_URL')}/img/$avatar"),
                                        fit: BoxFit.cover))),
                            const SizedBox(
                              width: 8,
                            ),
  ...
  Future skipLikeClick(int id) async {
    final result =
        await Provider.of<LikeState>(context, listen: false).skipLike(id);
    if (result) {
  ...
  Future thanksClick(int id) async {
    final result =
        await Provider.of<LikeState>(context, listen: false).createMatching(id);
    if (result) {
      Navigator.pushNamed(context, "/likelist");
    }
  }
  ...
  Future addBlock() async {
    final result = await Provider.of<BlockState>(context, listen: false)
        .addBlock(userById.id);
    if (result) {
      Navigator.pop(context);
      Navigator.pop(context);
    }
                      ...
                      children: [
                        Container(
                          ...
                          child: MaterialButton(
                            onPressed: () {
                              Navigator.pop(context);
                              Navigator.pushNamed(context, "/violation_screen",
                                  arguments: ChattingTransferModel(
                                      userById.id,
                                      userById.name,
                                      userById.avatars[0],
                                      userById.prefectureId,
                                      userById.age));
                            },
                            child: Center(
                              child: CustomText(
                      ...
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                              border: Border(
                                  top: BorderSide(
                                      color: AppColors.primaryGray, width: 1.5),
                                  right: BorderSide(
                                      color: AppColors.primaryGray,
                                      width: 1.5))),
                          child: TextButton(
                        ...
                        child: Stack(
                          children: [
                            Center(
                              child: CustomContainer(
                                ...
                                decoration: BoxDecoration(
                                    image: DecorationImage(
                                        image: NetworkImage(
                                            "${dotenv.get('BASE_URL')}/img/$currentAvatar"),
                                        fit: BoxFit.cover)),
                              ),
                            ),
                            Center(
                                        ...
                                        children: [
                                          ...
                                          Column(
                                            children: [
                                              ...
                                              ProfileItem(
                                                  label: "身長",
                                                  text: "${userById.height}cm"),
                                              ProfileItem(
                                                  label: "体型",
                                                  text: ConstFile.bodyTypes[
                                                      userById.bodyType]),
                                              ProfileItem(
                                                  label: "血液型",
                                                  text: ConstFile.bloodItems[
                                                      userById.blood]),
                                              ProfileItem(
                                                  label: "出身地",
                                                  text:
                                              ...
                                              ProfileItem(
                                                  label: "学歴",
                                                  text:
                                                      ConstFile.educationItems[
                                                          userById.education]),
                                              ProfileItem(
                                                  label: "職種",
                                                  text: ConstFile.jobTypes[
                                                      userById.jobType]),
                                              ProfileItem(
                                                  label: "年収",
                                                  text: ConstFile.incomes[
                                                      userById.income]),
                                              if (userById.holiday != null)
                                                ProfileItem(
                                                    label: "休日",
                                                    text: ConstFile.holidayItem[
                                                        userById.holiday!]),
                                              if (userById.roomate != null)
                                                ProfileItem(
                                                    label: "同居人",
                                                    text: ConstFile.roomateItem[
                                                        userById.roomate!]),
                                              if (userById.alcohol != null)
                                                ProfileItem(
                                                    label: "お酒",
                                                    text: ConstFile.alcoholItem[
                                                        userById.alcohol!]),
                                              if (userById.smoking != null)
                                                ProfileItem(
                                                    label: "タバコ",
                                                    text: ConstFile.smokingItem[
                                                        userById.smoking!]),
                                              ProfileItem(
                                                  label: "結婚歴",
                                                  text: ConstFile
                                              ...
                                              ProfileItem(
                                                  label: "結婚に対する意思",
                                                  text: ConstFile.attitudes[
                                                      userById.attitude]),
                                              ProfileItem(
                                                  label: "子供の有無",
                                                  text: ConstFile.childrenItems[
                                                      userById.children]),
                                              ProfileItem(
                                                  label: "家事や育児",
                                                  text:
                                                      ConstFile.houseworkItems[
                                                          userById.housework]),
                                              ProfileItem(
                                                  label: "出会うまでの希望",
                                                  text: ConstFile.hopeMeetItems[
                                                      userById.hopeMeet]),
                                              ProfileItem(
                                                  label: "初回デート費用",
                                                  text: ConstFile.dateCostItems[
                                                      userById.dateCost]),
                                              if (userById.saving != null)
                                                ProfileItem(
                                                    label: "貯蓄",
                                                    text: ConstFile.savingsItem[
                                                        userById.saving!]),
                                            ],
                                          )
                                        ],
                                              ...
                                              children: [
                                                Container(
                                                  ...
                                                  decoration: BoxDecoration(
                                                      ...
                                                      image: DecorationImage(
                                                          image: NetworkImage(
                                                              "${dotenv.get('BASE_URL')}/img/${userById.favoriteImage}"),
                                                          fit: BoxFit.cover)),
                                                )
                                              ],
                                            ),
                                            if (userById.favoriteDescription !=
                                                null)
                                              Container(
                            ...
                            Center(
                              child: Padding(
                                padding:
                                    const EdgeInsets.only(top: 32, left: 10),
                                child: SizedBox(
                                  height: 100,
                                  child: Row(
                                    mainAxisAlignment:
...
class ProfileItem extends StatelessWidget {
  const ProfileItem({super.key, required this.label, required this.text});
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            border: Border(
                bottom: BorderSide(color: AppColors.primaryGray, width: 1))),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          crossAxisAlignment: CrossAxisAlignment.center,
          children: [
